...
const extractCSSWrapper = (function extractCSSWrapperFactory(extractCSSArg, isDevArg) {
  return function extractCSSWrapperInner(loaderDefinition) {
    ...
    return (isDevArg) ? prefix + loaderDefinition : 
      extractCSSArg.extract(loaderDefinition);
  };
}(extractCSS, isDev));
...
const config = {
  ...
  plugins: [
    ...
    //   {
    //     ignore: [
    //       // ignore computed style files and jade templates, but not js files
